...
//ring parKeyword2 = cmR.convertNullToString(request.getParameter("inpKeyword2"));	/* 文字列検索Word */
...
//ring parKeywordW2 = "";												/* where句ワークエリア2 */
String strTitleBar = "BookDB: ";	/* タイトルバー */
...
String parDistinct = "DISTINCT vol_id,seq,title,title_seq,title_seq_sin,"
                   ...
                   + "author2_sin,author3_sin,author4_sin,publish_id,isbn,price,cur_sin,"
                   + "url_a,img_a,url_b,img_b,url_c,img_c,url_e,img_e,img_sin,store_sin,media_sin,"
                   + "memo,get_date,get_date_flg,get_yet_flg,modify_date";
String parSelectW = "ORDER BY title_sort,title_seq,vol_id,seq";
/*String parUpdate1 = "update book_t set";
String parInsert1 = "insert into book_t set";
String parInsertW = "insert into book_w set";
String parDelete1 = "delete from book_t";
*/
//ring parSelectTitle = "select * from title_m";
//ring parSelectPlayer = "select * from player_m";
...
//	Statement st=db.createStatement();		//１件検索用
//	Statement stMod=db.createStatement();		//BookDB更新用
//	Statement st2=db.createStatement();		//明細検索
//	Statement stM=db.createStatement();		//マスタ検索用
//	Statement stM2=db.createStatement();		//マスタ検索結果でBookDB検索
//	Statement stW=db.createStatement();		//ワークTable検索用
//	Statement stWMod=db.createStatement();		//ワークTable格納用
/* Query組み立て */
...
//	StringBuffer query4;
	...
	/* [購入日]の再検索 */
	if (sch_btn.equals("btnGetDate")) {
		query = new StringBuffer();
		query.append("WHERE get_date = '");
		if (parKeyword.equals("")) {			// 購入日が空なら本日付検索
			parKeyword = nowDate;
		}
		query.append(parKeyword).append("' ORDER BY title_sort, vol_id, seq");
	}
%>
<%
	/* 文字列[検索]で検索 */
...
//		if ((parKeyword.equals("")) && (parKeyword2.equals(""))) {
...
//			parKeywordW2 = "%" + escapeString(parKeyword2) + "%";
		...
		//次画面のタイトルバー設定
		sBfTitleBar.append(strTitleBar).append(parKeywordW);
...
*/
		...
		if (parKeywordT.equals("T")) {
			//タイトルでBookDB直読み
//			if (parKeyword2.equals("")) {		//出版社ブランクならBookDB直読み
			if (parKeywordM.equals("EQ")) {
			...
			query.append(" ORDER BY title_sort, vol_id, seq");
		} else if (parKeywordT.equals("P")) {
			//演者名でPlayerマスタ読み
//			query = new StringBuffer();
			if (parKeywordM.equals("EQ")) {
//				if (parKeyword.equals("")) {
			...
			query.append(" ORDER BY last_sort, first_sort, family_sort");
		} else if (parKeywordT.equals("H")) {
			//出版社でTitleマスタ読み
			if (parKeywordM.equals("EQ")) {
				query.append("WHERE title = '").append(parKeywordW).append("'");
			...
			query.append(" ORDER BY title_sort, subtitle_sort");
/*//出版社のふりがなを検索
		if (parKeywordT.equals("TS")) {
			query = new StringBuffer();
			if (parKeyword2.equals("")) {		//出版社ブランクならBookDB直読み
				if (parKeywordM.equals("EQ")) {
				...
				query.append(" ORDER BY title_sort, vol_id, seq");
			} else {							//出版社指定なら出版社でTitleマスタ読み
				if (parKeywordM.equals("EQ")) {
					query.append("WHERE title_sort = '").append(parKeywordW2).append("'");
					query.append(" OR subtitle_sort = '").append(parKeywordW2).append("'");
				} else {
					query.append("WHERE title_sort LIKE '").append(parKeywordW2).append("'");
					query.append(" OR subtitle_sort LIKE '").append(parKeywordW2).append("'");
				}
				query.append(" ORDER BY title_sort, subtitle_sort");
			}
		}
		...
		if (parKeywordT.equals("P")) {
			...
			if (parKeywordM.equals("EQ")) {
				if (parKeyword.equals("")) {
					query.append("WHERE first_name = '").append(parKeywordW2).append("'");
					query.append(" OR family_name = '").append(parKeywordW2).append("'");
				} else {
					query.append("WHERE last_name = '").append(parKeywordW).append("'");
					if (parKeyword2.equals("")) {
						query.append(" OR family_name = '").append(parKeywordW).append("'");
					} else {
						query.append(" AND first_name = '").append(parKeywordW2).append("'");
					}
				}
			} else {
				if (parKeyword.equals("")) {
					query.append("WHERE first_name LIKE '").append(parKeywordW2).append("'");
					query.append(" OR family_name LIKE '").append(parKeywordW2).append("'");
				} else {
					query.append("WHERE last_name LIKE '").append(parKeywordW).append("'");
					if (parKeyword2.equals("")) {
						query.append(" OR family_name LIKE '").append(parKeywordW).append("'");
					} else {
						query.append(" AND first_name LIKE '").append(parKeywordW2).append("'");
					}
				}
			}
			query.append(" ORDER BY last_name, first_name, family_name");
		}
		//プレイヤのふりがなを検索
		if (parKeywordT.equals("PS")) {
			query = new StringBuffer();
			if (parKeywordM.equals("EQ")) {
				if (parKeyword.equals("")) {
					query.append("WHERE first_sort = '").append(parKeywordW2).append("'");
					query.append(" OR family_sort = '").append(parKeywordW2).append("'");
				} else {
					query.append("WHERE last_sort = '").append(parKeywordW).append("'");
					if (parKeyword2.equals("")) {
						query.append(" OR family_sort = '").append(parKeywordW).append("'");
					} else {
						query.append(" AND first_sort = '").append(parKeywordW2).append("'");
					}
				}
			} else {
				if (parKeyword.equals("")) {
					query.append("WHERE first_sort LIKE '").append(parKeywordW2).append("'");
					query.append(" OR family_sort LIKE '").append(parKeywordW2).append("'");
				} else {
					query.append("WHERE last_sort LIKE '").append(parKeywordW).append("'");
					if (parKeyword2.equals("")) {
						query.append(" OR family_sort LIKE '").append(parKeywordW).append("'");
					} else {
						query.append(" AND first_sort LIKE '").append(parKeywordW2).append("'");
					}
				}
			}
			query.append(" ORDER BY last_sort, first_sort, family_sort");
...
//		if ((parKeywordT.indexOf("T") >= 0) && (parKeyword2.equals(""))) {
		if (parKeywordT.equals("T")) {
			//タイトルのみ指定のときマスタ検索しない。
		} else if (parKeywordT.equals("D")) {
			//購入日指定のときマスタ検索しない。
//			parKeyword2 = "";
			...
			//出版社指定ありまたは著者検索のときマスタ検索する。
			useMainDB = false;						//後段でワークTableを読ませる。
			if (parKeywordT.equals("H")) {
...
//			out.println("<br />マスタ件数:" + idx);
			//book_wを全件削除
			bwU.deleteRec("", -1);
			for (int j = 0; j < idx; j++) {
			//タイトル・プレイヤ マスタのIDでBookDBを検索
...
<!-- VolSeq制御列 -->
	<tr align="center" valign="middle">
		...
		<!--td align="left" bgcolor="#cccccc"
			id="thBkKeyword">版元/名</td-->
		<td class="fontSmall" id="thBkKeyword">
			<!--input size="12" type="text" maxlength="255" name="inpKeyword2"
				value="">
		</td>
		<td align="left"-->
		<%
//			out.println(cmF.makeSelectItem("selKeywordT", "selKeywordT", parKeywordT));
			strCo = new String[10];
			if (parKeywordT.equals("T")) { strCo[0] = "checked"; }
			if (parKeywordT.equals("P")) { strCo[1] = "checked"; }
			if (parKeywordT.equals("H")) { strCo[2] = "checked"; }
			if (parKeywordT.equals("D")) { strCo[3] = "checked"; }
		%>
		<!--/td>
		<td align="center"-->
			<input type="radio" name="rdoKeyword" value="T" <%= strCo[0] %>>書名
			<input type="radio" name="rdoKeyword" value="P" <%= strCo[1] %>>著者
			<input type="radio" name="rdoKeyword" value="H" <%= strCo[2] %>>出版社
			<input type="radio" name="rdoKeyword" value="D" <%= strCo[3] %> id="rdoKeywordD">購入日
		</td>
		<td align="center">
			<input type="button" onclick="javascript:openCalendarWindow('BkKeyword')"
				name="btnGetDate" value="購入日">
		</td>
		<td>
	...
	<tr>
		...
		<td>
			...
			<input type="button" name="btnPriceClear"
				onclick="javascript: clearInpData('price')" value="クリア">
		</td>
