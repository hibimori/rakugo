...
/* マスタ rewrite */
	...
	if (goFlg == true) {
		...
		document.formBook.action = "viewbook.jsp";
...
<hr>
<div class="divFloatR">
	<textarea name="txtHTML" rows="23" cols="48"></textarea>
</div>
...
<hr>
<div>
<textarea class="ckeditor" id="editor" name="editor" cols="128" rows="8"></textarea>
</div>
<hr>
<%!
...
*/
	...
	*/
	...
	if (tarDate.get(Calendar.YEAR) > 2010) {
		
	hdtTable.append("<div class='dateTable").append(hdtW);
	hdtTable.append("' id='").append(hdtID).append("'>");
	...
	hdtTable.append("<span class='dateJulian fontSmall'>&nbsp;");
	hdtTable.append("<a href='").append(hdtPath).append(dateFmtYM.format(hdtDate)).append(".html");
	hdtTable.append("#d").append(dateFmtD.format(hdtDate)).append("'>");
	hdtTable.append(hdtJ).append("</a>&nbsp;</span>");
	//a nameタグID生成
	hdtID = "d" + dateFmtD.format(hdtDate);
	//TOP/BOTTOMリンク記述
	hdtTable.append("<span class='dateLink fontSmall'>");
	hdtTable.append("<a name='").append(hdtID).append("' id='").append(hdtID);	//当日id
	hdtTable.append("'></a>［<a href='#b99'>▽</a>｜<a href='#b0'>△</a>］</span>");
	//日曜日特例
	if (tarDate.get(Calendar.DAY_OF_WEEK) == 1) {
		...
		hdtTable.append(tarWw).append("kg</a></span>");
	}
	//１日特例
	if (tarDate.get(Calendar.DATE) == 1) {
		hdtTable.append("<span class='fontSmall'>148,000</span>");
	}
	hdtTable.append("</div>");
	
	} else {
		
	hdtTable.append("<table border='0' class='dateTable").append(hdtW);
	hdtTable.append("' id='").append(hdtID).append("'>");
	hdtTable.append("<tbody><tr><th class='date").append(hdtW);
	hdtTable.append("' onclick='javascript:showCalendar(\\\"");
	hdtTable.append(hdtID).append("\\\");'");
	hdtTable.append(" style='cursor:pointer;'>");
	hdtTable.append(dateFmtS.format(hdtDate)).append("</th>");
	hdtTable.append("<td class='dateJulian fontSmall'>");
	hdtTable.append("<a href='").append(hdtPath).append(dateFmtYM.format(hdtDate)).append(".htm");
	hdtTable.append("#d").append(dateFmtD.format(hdtDate)).append("'>");
	hdtTable.append(hdtJ).append("</a></td>");

	//a nameタグID生成
	hdtID = "d" + dateFmtD.format(hdtDate);
	//TOP/BOTTOMリンク記述
	hdtTable.append("<td class='dateLink fontSmall'>");
	hdtTable.append("<a name='").append(hdtID).append("' id='").append(hdtID);	//当日id
	hdtTable.append("'></a>［<a href='#b99'>▽</a>｜<a href='#b0'>△</a>］</td>");
/*	hdtTable.append("'></a>［<a href='#d").append(dateFmtD.format(hdtDateP)).append("'>＜</a>");
	hdtTable.append("<a href='#d").append(dateFmtD.format(hdtDateN)).append("'>＞</a>");
	*/
	//日曜日特例
	if (tarDate.get(Calendar.DAY_OF_WEEK) == 1) {
		hdtTable.append("<td class='fontSmall'>");
		hdtTable.append("<a href='");
/*	hdtTable.append("http://graph.hatena.ne.jp/detail?id=nii&graphname=since%201998&id=nii&graphname=%E6%BF%80%E7%97%A9%E3%81%9B%E5%B9%B4%E9%96%932005");
*/
		hdtTable.append("http://graph.hatena.ne.jp/nii/since%201998/");
		hdtTable.append("' target='niin'>");
		hdtTable.append(tarWw).append("kg</a></td>");
	}
	//１日特例
	if (tarDate.get(Calendar.DATE) == 1) {
		hdtTable.append("<td class='fontSmall'>168,000</td>");
	}
	hdtTable.append("</tr></tbody></table>");
	}
...
public String aiboDateTable(Calendar tarDate, Calendar tarDateA, Calendar tarDateB) {
	...
	SimpleDateFormat dateFmtD = new SimpleDateFormat("d");
	//日記htm置き場（2011.1 - 2015.12）
	String adtPath = "file:///d:/webber/nikkl/";
	...
	//今回分のTable
//	adtTable.append("<div class='aiboDiv'><div><span class='fontSmall'>");
	adtTable.append("<div><span class='fontSmall'>");
	...
	adtTable.append("＜</a><a name='aibo").append(dateFmtYMD.format(adtDate)).append("'></a>aiBo</span>");
	adtTable.append("<span class='fontXSmall'>").append(dateFmtYMD.format(adtDate)).append("</span>");
	adtTable.append("<span class='fontSmall'>＞】</span></div>");
	//前回分のTable
	adtTable.append("<p>。</p><div><span class='fontSmall'>");
	...
	adtTable.append("＜</a><a name='aibo").append(dateFmtYMD.format(adtDateB)).append("'></a>aiBo</span>");
	adtTable.append("<span class='fontXSmall'>").append(dateFmtYMD.format(adtDateB)).append("</span>");
	adtTable.append("<span class='fontSmall'>");
	adtTable.append("<a href='").append(adtPath).append(dateFmtYM.format(adtDate)).append(".html");
	adtTable.append("#aibo").append(dateFmtYMD.format(adtDate)).append("'>＞</a>】</span></div>");
	adtTable.append("<p>。</p><p>。</p>");
//	adtTable.append("<p>。</p><p>。</p></div>");
...
// 日記雛型記述開始
	...
	if (sch_chkViewF.equals("1")) {		//ブランク日付のあるとき
		...
		sbfHTML.append("<div id='dr").append(dateFmtID.format(wkDate3)).append("skip'>");
		sbfHTML.append("<div class='dailyDiv").append(htmW).append("2'>");
...
<%
	...
	if (!(sch_selViewN.equals("2"))) {		//書影のみのとき日付なし
		sbfHTML.append("<div id='dr").append(dateFmtID.format(wkDate3)).append("'>");
		sbfHTML.append("<div class='dailyDiv").append(htmW).append("2'>");
...
<%
	...
	if (sch_chkViewA.equals("1")) {
		...
		sbfHTML.append("<hr class='hr50 hr").append(htmW).append("'>");
	}
%>
<%
	//日記本文部記述
	if (sch_selViewN.equals("2")) {
		sbfHTML.append("<hr width='32%'");
		sbfHTML.append(" class='hr50 hr").append(htmW).append("'>");
	} else {										//書影のみのとき空段落なし
		for (i = 1; i < 4; i++) {
			sbfHTML.append("<a name='d").append(nowD).append("a").append(i);
			sbfHTML.append("' id='d").append(nowD).append("a").append(i).append("'></a>");
			sbfHTML.append("<ul class='listSq'><li>　</li></ul><p>。</p><p>。</p>");
			sbfHTML.append("<hr class='hr50 hr").append(htmW).append("'>");
		}
		sbfHTML.append("<a name='d").append(nowD).append("a").append(i);
		sbfHTML.append("' id='d").append(nowD).append("a").append(i).append("'></a>");
		sbfHTML.append("<p>。</p><p>。</p>");
	}
	//書影Table記述（【きょうの愉しみ】)
	int wkCol = 0;
	int wkRow = 0;
	...
	if ((wkCol > 0) && (!(sch_selViewN.equals("1")))) {		//書影のあるときのみ
		if (!(sch_selViewN.equals("2"))) {		//書影のみのときタイトルなし
			sbfHTML.append("<hr class='hr50 hr").append(htmW).append("'>");
/*			sbfHTML.append("<div>【<a href='http://graph.hatena.ne.jp/nii/graph?graphname=");
			...
			sbfHTML.append("' target='niin'>きょうの愉しみ</a>】</div>");
			*/
			sbfHTML.append("<div><a name='d").append(nowD).append("kt' id='d").append(nowD);	//当日id
			sbfHTML.append("kt'></a>【きょうの愉しみ】</div>");
		}
		...
		sbfHTML.append("<div class='divFloatR'><table border='0' class='fontSmall'><tbody>");
		cntCell = 1;
		for (nowRow = 1; nowRow <= wkRow; nowRow++) {		//行ループ
			sbfHTML.append("<tr class='bookPhoto' valign='bottom'>");
			for (nowCol = 1; nowCol <= wkCol; nowCol++) {	//列ループ
				if (nowCol < wkCol - (rowsImg - cntCell)) {
					...
					if ((rowsImg == 6) && (nowRow == 2) && (nowCol == 1)) {
						...
						sbfHTML.append("<td nowrap='nowrap'>");
						...
						sbfHTML.append("' border='0' src='").append(aryImg[idxImg]);
						if (wkCol > 2) {
						...
						//A8.net専用のダミーimg
							if (aryURLB[idxImg].indexOf("a8.net") >= 0) {
								sbfHTML.append("<img border='0' width='1' height='1' src='");
