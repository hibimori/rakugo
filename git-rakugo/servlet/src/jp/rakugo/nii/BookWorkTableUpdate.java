 ...
 *
 */
public class BookWorkTableUpdate implements Serializable {
//書籍ワークテイブル更新
	...
	public BookWorkTableUpdate() {}		//コンストラクタ
	...
	public int insertRec() {
		try {
			...
			StringBuffer q = new StringBuffer("INSERT INTO book_w SET ");
	...
	public int updateRec() {
		try {
			...
			StringBuffer q = new StringBuffer("UPDATE book_w SET ");
			q.append(makeParameter());
	...
	public int deleteRec(String id, int seq) {
		try {
			...
			StringBuffer q = new StringBuffer("DELETE FROM book_w");
			if ((! id.equals("")) &&
			    (seq < 0)) {
				//id, seq指定なければ全件削除
