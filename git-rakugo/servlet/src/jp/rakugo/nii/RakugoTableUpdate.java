 ...
 *
 */
public class RakugoTableUpdate implements Serializable {
//書籍テイブル更新
	...
	public RakugoTableUpdate() {}		//コンストラクタ
	...
	public int insertRec() {
		try {
			...
			StringBuffer q = new StringBuffer("INSERT INTO rakugo_t SET ");
	...
	public int updateRec() {
		try {
			...
			StringBuffer q = new StringBuffer("UPDATE rakugo_t SET ");
			q.append(makeParameter());
	...
	public int deleteRec(String id, int seq) {
		try {
			...
			StringBuffer q = new StringBuffer("DELETE FROM rakugo_t");
	...
	private String makeParameter() {
		...
		if (parRec.rec_length != null) {
			stB.append(",rec_length='");
			try {
				stB.append(timeFmt.format(parRec.rec_length));
			} catch (Exception e) {
				stB.append("00:00:00");
			}
			stB.append("'");
		...
		if (parRec.rec_date != null) {
			stB.append(",rec_date='");
			try {
				stB.append(dateFmt.format(parRec.rec_date));
			} catch (Exception e) {
		...
		if (parRec.rec_time != null) {
			stB.append(",rec_time='");
			try {
				stB.append(timeFmt.format(parRec.rec_time));
			} catch (Exception e) {
				stB.append("00:00:00");
			}
			stB.append("'");
